# Painting Service Square Foot Pricing Reference

**Status:** Draft
**Date:** February 2026

## 1. Multipliers & Project Variables

| Item | Value | Unit | Notes |
| :--- | :--- | :--- | :--- |
| **Hourly Labor Rate** | **$75.00** | /hour | Base rate for all calculations. |
| **Occupancy: Empty/New** | **$0.00** | /room | No furniture. |
| **Occupancy: Owner Clears** | **$0.00** | /room | Owner moves/covers everything. |
| **Occupancy: Painter Moves** | **$60.00** | /room | Painter moves/covers heavy furniture. |
| **Ceiling Height: 8-9'** | **1.00x** | multiplier | Standard. |
| **Ceiling Height: 10'** | **1.10x** | multiplier | Moderate height. |
| **Ceiling Height: 12-14'** | **1.25x** | multiplier | High. |
| **Ceiling Height: 15'+** | **1.45x** | multiplier | Vaulted. |

### Equipment Rental
*   **Trigger:** Automatically applied for 12ft+ ceilings.
*   **Scaffolding:** **$150.00** flat fee per day.
*   **Ladder (26ft):** **$60.00** flat fee per day.

---

## 2. Base Production Rates (Walls & Ceilings)
*Note: Prices derived from $75/hr labor rate.*

| System | Speed (Avg) | Labor Price | Description |
| :--- | :--- | :--- | :--- |
| **1 Coat System** | 130 sqft/hr | **$0.58** /sqft | Refresh (Same Color). |
| **2 Coat System** | 80 sqft/hr | **$0.94** /sqft | Color Change. |
| **3 Coat System** | 55 sqft/hr | **$1.36** /sqft | Dark-to-Light (Prime + 2). |

### Surface Preparation (Additive)
| Condition | Rate | Labor Price | Notes |
| :--- | :--- | :--- | :--- |
| **None** | 0.00 hrs/100 | **$0.00** /sqft | No extra prep. |
| **Good** | 0.15 hrs/100 | **$0.11** /sqft | Minor nail holes. |
| **Fair** | 0.40 hrs/100 | **$0.30** /sqft | Scuffs, dings, light patching. |
| **Poor** | 1.00 hr/100 | **$0.75** /sqft | Extensive patching, cracks. |

### Ceiling Texture Surcharges (Additive %)
*Note: Labor surcharge is applied to the ceiling's Base Production Rate. Material multiplier is applied to the ceiling's base paint volume.*

| Texture | Labor Surcharge | Material Multiplier | Notes |
| :--- | :--- | :--- | :--- |
| **Smooth** | **0%** | **1.00x** | Standard flat/smooth finish. |
| **Orange Peel / Knockdown**| **+15%** | **1.30x** | (+30% extra paint) |
| **Popcorn** | **+40%** | **2.00x** | (+100% extra paint) |

---

## 3. Base Production Rates (Trim & Molding)
*Note: Trim and Crown Molding are high-detail, slow-paced tasks involving extensive masking and precision cutting. Consequently, labor prices for these items are derived from a **$100.00/hr** effective rate rather than the standard $75.00/hr.*

| System | Speed (Avg) | Labor Price | Description |
| :--- | :--- | :--- | :--- |
| **Trim - 1 Coat** | 60 lf/hr | **$1.67** /lf | Refresh. |
| **Trim - 2 Coats** | 35 lf/hr | **$2.86** /lf | Standard / Color Change. |
| **Trim - 3 Coats** | 20 lf/hr | **$5.00** /lf | Prime + 2 (Stain-to-Paint). |
| **Crown - 1 Coat** | 40 lf/hr | **$2.50** /lf | Refresh. |
| **Crown - 2 Coats** | 25 lf/hr | **$4.00** /lf | Standard / Color Change. |
| **Crown - 3 Coats** | 15 lf/hr | **$6.67** /lf | Prime + 2 (Stain-to-Paint). |

---

## 4. Fixed Item Production Rates

| Item | Rate | Labor Price | Notes |
| :--- | :--- | :--- | :--- |
| **Door (Slab)** | 1.0 hr | **$75.00** ea | Both sides & frame. |
| **Door (Paneled)** | 1.5 hr | **$112.50** ea | Both sides & frame. |
| **Window Frame** | 0.5 hr | **$37.50** ea | Standard casing/sash. |
| **Closet (Standard)**| 1 hr | **$75.00** ea | 2' x 4' reach-in. |
| **Closet (Medium)** | 1.5 hr | **$112.50** ea | 5' x 5' walk-in. |
| **Closet (Large)** | 2 hr | **$150.00** ea | 6' x 10' walk-in. |

---

## 5. Stairwells

| Item | Time | Labor Price | Unit |
| :--- | :--- | :--- | :--- |
| **Square Spindle** | 12 mins | **$15.00** | each |
| **Intricate Spindle** | 30 mins | **$37.50** | each |
| **Handrail** | 6 lf/hr | **$12.50** | /lf |
| **Risers & Stringers** | 20 mins | **$25.00** | /step |

---

## 6. Paint & Material Pricing
*Note: Material prices are per gallon and do not include the 15% waste buffer.*

| Tier | Price/Gal | Notes |
| :--- | :--- | :--- |
| **Pro-Base** | **$40.00** | Essential finish. |
| **Standard** | **$55.00** | Home signature. |
| **Premium** | **$85.00** | Elite Performance. |
| **Ultra Premium** | **$128.00** | Masterpiece Finish. |
| **Universal Primer** | **$45.00** | |

---

## 7. Material Yields (Per Coat)
*Apply 15% waste buffer to all final volumes.*

| Item | Yield | Unit |
| :--- | :--- | :--- |
| **Wall/Ceiling Paint**| **350** | sqft/gal |
| **Primer** | **250** | sqft/gal |
| **Trim Paint** | **700** | lin.ft/gal |
| **Doors (Both Sides)**| **8** | doors/gal |
| **Windows** | **12** | windows/gal | (1 quart per 3 windows) |

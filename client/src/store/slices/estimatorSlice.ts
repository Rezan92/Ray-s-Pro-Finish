...
const createNewRoom = (type: string, id: string, label: string): PaintingRoom => {
...
export const estimatorSlice = createSlice({
  ...
  reducers: {
    ...
    // Service Toggles
    toggleService: (state, action: PayloadAction<{ name: keyof FormData['services']; checked: boolean }>) => {
      state.formData.services[action.payload.name] = action.payload.checked;
    },
    
    // --- Painting Logic ---
    toggleRoomType: (state, action: PayloadAction<{ type: string; isChecked: boolean }>) => {
      const { type, isChecked } = action.payload;
      if (isChecked) {
        // Add first room if not exists
        const exists = state.formData.painting.rooms.some((r) => r.type === type);
        if (!exists) {
          const newId = `${type}_0`;
          const isMulti = type === 'bedroom' || type === 'bathroom';
          const newLabel = isMulti ? `${ROOM_LABELS[type]} 1` : ROOM_LABELS[type];
          state.formData.painting.rooms.push(createNewRoom(type, newId, newLabel));
        }
      } else {
        // Remove all rooms of this type
        state.formData.painting.rooms = state.formData.painting.rooms.filter((r) => r.type !== type);
      }
    },
    addRoom: (state, action: PayloadAction<string>) => {
      const type = action.payload;
      const count = state.formData.painting.rooms.filter((r) => r.type === type).length;
      const newId = `${type}_${count}`;
      const newLabel = `${ROOM_LABELS[type]} ${count + 1}`;
      state.formData.painting.rooms.push(createNewRoom(type, newId, newLabel));
    },
    removeRoom: (state, action: PayloadAction<string>) => {
      state.formData.painting.rooms = state.formData.painting.rooms.filter((r) => r.id !== action.payload);
    },
    updateRoomField: (state, action: PayloadAction<{ roomId: string; field: string; value: any }>) => {
      const room = state.formData.painting.rooms.find((r) => r.id === action.payload.roomId);
      if (room) {
        // @ts-ignore - Dynamic assignment is safe here due to payload structure
        room[action.payload.field] = action.payload.value;
      }
    },
    updatePaintingGlobal: (state, action: PayloadAction<{ field: string; value: string }>) => {
        // @ts-ignore
      state.formData.painting[action.payload.field] = action.payload.value;
    },

    // --- Generic Nested Updates (Patching/Installation) ---
    updateNestedForm: (state, action: PayloadAction<{ path: 'patching' | 'installation'; field: string; value: any }>) => {
      // @ts-ignore
      state.formData[action.payload.path][action.payload.field] = action.payload.value;
    },
